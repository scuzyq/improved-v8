...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_improve, Detect_AFPN, Detect_ASFF, Detect_FRM, ImplicitHead, AsDDet, LADH, Detect_DBB, Detect_AFPN3, CLLAHead, Detect_FASFF, Detect_Adown, Detect_DySnakeConv, Detect_LSDECD, Detect_LADH, Detect_dyhead3, Segment)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolov8n.yaml', ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Detect_improve, Detect_AFPN, Detect_ASFF, Detect_FRM, AsDDet,  ImplicitHead, LADH, Detect_DBB, Detect_AFPN3, CLLAHead, Detect_FASFF, Detect_Adown, Detect_DySnakeConv, Detect_LSDECD, Detect_LADH, Detect_dyhead3, Segment, Pose)):
            s = 256  # 2x min stride
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_improve, Detect_AFPN, Detect_ASFF, Detect_FRM, AsDDet, ImplicitHead, LADH, Detect_DBB, Detect_AFPN3, CLLAHead, Detect_FASFF, Detect_Adown, Detect_DySnakeConv, Detect_LSDECD, Detect_LADH, Detect_dyhead3, Segment):
            m.inplace = inplace
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, SPDConv, DWConv, Focus,
            ...
            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, C2f_Att, C2f_DCN, C2f_UIB, C2f_deformable_LKA, SKAttention, C2f_DySnakeConv,EMA_attention, C2f_MSBlock, C2f_MLCA, C2f_DCNv3_DLKA, C2fMLLABlock, C2f_Dual, C2f_DCN2, C2f_KAN):
            ...
            c2 = ch[f[1]]
            args = [c2, *args]
        elif m in (Detect, Detect_improve, Detect_AFPN, Detect_ASFF, Detect_FRM, AsDDet, ImplicitHead, LADH, Detect_DBB, Detect_AFPN3, CLLAHead, Detect_FASFF, Detect_Adown, Detect_DySnakeConv, Detect_LSDECD, Detect_LADH, Detect_dyhead3, Segment, Pose):
...
def guess_model_task(model):
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Detect, Detect_improve, Detect_AFPN, Detect_ASFF, Detect_FRM, AsDDet, ImplicitHead, LADH, Detect_DBB, Detect_AFPN3, CLLAHead, Detect_FASFF, Detect_Adown, Detect_DySnakeConv, Detect_LSDECD, Detect_LADH, Detect_dyhead3)):
                return 'detect'
